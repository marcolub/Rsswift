...
//    
    // initilise parser
    func initWithURL(_ url :[String]) -> AnyObject {
        startParse(url)

        //let descriptor = NSSortDescriptor(key: "pubDate", ascending: false)
        //feeds.sort(using: [descriptor])

        return self
    }
    
    func startParse(_ urlArray :[String]) {
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String]) {
        ...
        if (element as NSString).isEqual(to: "item") {
            ...
            if let urlString = attributeDict["url"] {
                ...
                elements.setObject(urlString, forKey: "image" as NSCopying)
            }
        }
    }
